...
IonicTask.prototype.run = function (ionic) {
    ...
    this.ionicAppId = argv['io-app-id'];
...
IonicTask.prototype.fetchWrapper = function () {
    ...
    var repoUrl = 'https://github.com/dezynetechnologies/' + IonicTask.WRAPPER_REPO_NAME + '/archive/master.zip';
...
IonicTask.prototype.fetchSeed = function () {
    ...
    if (/plnkr.co\//i.test(this.template)) {
        this.seedType = 'plnkr';
        return this.fetchPlnkr();
    }
...
IonicTask.prototype.fetchCodepen = function () {
    ...
    //Given any of these urls:
    // http://codepen.io/ionic/pen/QwamEW
    // http://codepen.io/ionic/live/QwamEW
    // http://codepen.io/ionic/details/QwamEW/
    // http://codepen.io/ionic/full/QwamEW/

    // we want to parse out and get the /pen/ID link if its not already a /pen link
    if (codepenUrl.indexOf('/pen') == -1) {
        if (codepenUrl[codepenUrl.length - 1] == '/') {
            codepenUrl = codepenUrl.substr(0, codepenUrl.length - 1);
        }
        var codePenSplit = codepenUrl.split('/')
        var codePenId = codePenSplit[codePenSplit.length - 1];
        var codePenOrg = codePenSplit[3];
        codepenUrl = ['http://codepen.io/', codePenOrg, '/pen/', codePenId].join('')
    }
...
IonicTask.prototype.fetchIonicStarter = function () {
    ...
    var repoUrl = 'https://github.com/dezynetechnologies/' + repoName;
...
IonicTask.prototype.initCordova = function () {
    ...
    if (this.isCordovaProject) {
        ...
        for (var x = 0; x < self.appSetup.plugins.length; x++) {
            cmds.push('ionic plugin add ' + self.appSetup.plugins[x]);
        }
...
IonicTask.prototype.updateConfigXml = function () {
    ...
    try {
        ...
        var configString = fs.readFileSync(configXmlPath, {
            encoding: 'utf8'
        });
...
IonicTask.prototype.finalize = function () {
    ...
    try {
        ...
        if (this.ionicAppId) {
            project.set('app_id', this.ionicAppId);
        }
...
IonicTask.prototype.fetchPlnkr = function fetchPlnkr() {
    var self = this;
    var q = Q.defer();

    var plnkrUrl = this.template.split('?')[0].split('#')[0];
    ...
    var extractPath = path.join(self.targetPath, 'plnkr');

    Ionic.fetchArchive(extractPath, plnkrUrl)
        .then(function () {
